...
var Transfer = (function ($) {
    ...
    function transfer(settings) {
        ...
        $(addSelected).on("click", function () {
            ...
            // 数据变化触发回调
            callable.call(this, companyId, companyName);
        });
        /**
         * 删除选中项，回到左侧
         */
    ...
    function generateTransfer(inputId, currentTimeStr) {
        var htmlStr =
            ...
            + '<div class="tab-item-name tab-item-name-' + currentTimeStr + ' tab-active">请选择企业</div>'
            + '</div>'
            ...
            + '<input class="transfer-double-list-search-input" type="text" ONBLUR="getCompanyList()" id="listSearch" placeholder="搜索" value="" />'
            + '</div>'
            + '</div>'
            ...
            + '<div class="btn-select-arrow" id="add_selected_' + currentTimeStr + '">></div>'
            + '</div>'
        ;
        return htmlStr;
    }
